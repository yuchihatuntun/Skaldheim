...
>     - **为什么？** 为了保护硬件。如果任何程序都能随意访问硬盘端口，它就能绕过文件系统，窃取或销毁所有数据。
...
1. **路径 ①**：通用寄存器 $\longleftrightarrow$ 通用寄存器
...
2. **路径 ② & ④**：段寄存器 $\longleftrightarrow$ 通用寄存器 或 存储器

	- **例**: `MOV BX, CS` / `MOV DS, [SI]`
	    
	- `CS`, `DS`, `SS`, `ES` 这些段寄存器非常特殊，它们掌管着内存寻址的“街道名称”。我们必须有办法设置它们。
	    
	- **路径**: 数据**只能从**“通用寄存器/存储器”**流向**“段寄存器”，或者**从**“段寄存器”**流向**“通用寄存器/存储器”。
    
- **规则**:
    
    1. `CS`（代码段）**不能作为目的操作数！** `MOV CS, AX` 是**非法**的。
    2. **为什么？** `CS` 和 `IP`（指令指针）是CPU的“发动机”，共同指向下一条要执行的指令。你不能用一个简单的数据传送指令去“篡改”发动机的指向。
    3. **如何修改CS？** 只能通过特定的**控制转移指令**（如`JMP FAR`或`CALL FAR`）来同时、安全地修改`CS`和`IP`。
    4. `CS` **可以作为源操作数**（如 `MOV BX, CS`），这允许程序查询自己当前的代码段地址。
        

3. **路径 ③**：通用寄存器 $\longleftrightarrow$ 存储器

- **例**: `MOV [BP+DI], DL` / `MOV AX, [BX]`
    
- 这是**最常见**的`MOV`操作。几乎所有程序都在做这两件事：
    
    1. **加载 (Load)**：`MOV AX, [BX]`，把内存中的数据，加载到寄存器中准备进行计算。
        
    2. **存储 (Store)**：`MOV [BP+DI], DL`，把寄存器中的计算结果，存回到内存中。

#### 路径 ⑤：立即数 $\longrightarrow$ 口袋 (立即数到通用寄存器)

- **PPT示例**: `MOV BX, 34H` 或 `MOV ESI, 12345678H`
    
- **讲解**: 这是我们给变量赋一个“常量”值。`34H` 这个数据（立即数）就编码在指令的机器码里。
    
- **工业扩展**: `int i = 123;` 几乎100%会被编译器翻译成 `MOV EAX, 123`。
    

#### 路径 ⑥：立即数 $\longrightarrow$ 仓库 (立即数到存储器)

- **PPT示例**: `MOV BYTE PTR [BX], 12H`
    
- **讲解**: 把一个常量（`12H`）直接存入一个内存地址（`[BX]`）。
    
- **`PTR` 关键字**：这是一个**类型说明符**。CPU需要知道你是想存一个字节(BYTE)、一个字(WORD)还是一个双字(DWORD)。
