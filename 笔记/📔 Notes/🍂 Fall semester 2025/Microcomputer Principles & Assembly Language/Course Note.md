...
- `DATA SEGMENT` 和 `DATA ENDS`
    
    - `ENDS` 为 `END Segment` 的缩写。
    
    - 这对指令指示汇编器：“从这里开始定义一个名叫`DATA`的数据段。以及，到‘DATA ENDS’这里，`DATA`数据段就定义结束了。”

- `CODE SEGMENT` 和 `CODE ENDS` 同理

- `STRING DB 'HELLO WORLD!', 0DH, 0AH, '$'`
    
    - 数据段里最关键的一行。`DB` 即 `Define Byte` (定义字节) 的缩写。
        
    - 此指令指示汇编器：“在内存里预留空间，命名为 `STRING`，然后按顺序在里面填入以下字节：”
        
        - `'HELLO WORLD!'`: 字符串的ASCII码。
            
        - `0DH`: 这是一个十六进制数，代表“回车”(Carriage Return)。它让光标回到一行的最左边。
            
        - `0AH`: 这是一个十六进制数，代表“换行”(Line Feed)。它让光标移动到下一行。
            
        - `'$'`: 这是DOS系统的一个“暗号”。后面我们要调用的DOS功能（`INT 21H` 的 `09` 号功能）规定，要打印的字符串必须以 `$` 符号结尾，它才知道“噢，打印到这里就结束了”。
            
- `ASSUME CS:CODE, DS:DATA`
    
    - 这是非常重要的一个“舞台提示”。它告诉汇编器：“请你_假设_：将来这个程序_真正运行_的时候，`CS`（代码段寄存器）会指向 ‘CODE’ 段，`DS`（数据段寄存器）会指向 ‘DATA’ 段。”
        
    - 汇编器有了这个假设，才能正确地计算出 `STRING` 的地址。
        
- `END BEGIN`
    
    - 这个`END`（注意和`ENDS`的区别）告诉汇编器：“整个源文件到此结束了。”
        
    - 后面的 `BEGIN` 告诉汇编器：“当这个程序被加载到内存中准备执行时，请告诉CPU从‘BEGIN’这个标签（我们下面会讲）所在的位置_开始执行第一条指令_。”