...
- 编译器**不会**执行 `MUL EAX, 10`。
    
- 编译器会把 `x * 10` 拆解为 `(x * 8) + (x * 2)`。
...
- 汇编：
    ...
    SHL EAX, 3     ; EAX = x * 8
    SHL EBX, 1     ; EBX = x * 2
    ADD EAX, EBX   ; EAX = (x * 8) + (x * 2) = x * 10
    ```
    
- **更高级的编译器**会使用`LEA`（加载有效地址）指令，一步到位： `LEA EAX, [ESI*8 + ESI*2]` (假设 `x` 在 `ESI` 中)
    
- **结论**：移位指令是现代编译器进行**强度折减 (Strength Reduction)** 优化的核心武器。
##### 字符串操作类 (String)
