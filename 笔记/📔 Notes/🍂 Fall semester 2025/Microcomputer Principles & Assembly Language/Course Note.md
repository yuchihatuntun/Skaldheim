...
**设置该段在存储器中的起始边界**，即<span style="background:rgba(74, 82, 199, 0.2)">起始物理地址</span>。这直接影响到段在内存中的布局和访问效率。

- `BYTE`：允许段从<span style="background:rgba(74, 82, 199, 0.2)">任意的绝对地址</span>开始。这是最宽松的定位方式，适用于对内存布局没有特殊要求的情况。

- `WORD`：要求段必须从<span style="background:rgba(74, 82, 199, 0.2)">偶地址</span>开始。因为x86架构的处理器在访问字（Word）数据时，如果数据位于偶地址，访问速度最快。这对于性能敏感的代码段或数据段非常重要。

- `DWORD`：要求段必须从双字（Double Word）的边界开始。进一步提高了对齐级别，适用于需要处理32位数据或在现代处理器上追求极致性能的场景。

- `PARA`（缺省方式）：这是默认的定位方式，要求段必须从16字节的边界开始。为了兼容早期的8086处理器，其内存管理单元（MMU）以16字节为单位进行分页。在现代系统中，虽然硬件不再强制要求，但为了保持兼容性和简化内存管理，`PARA`仍然是最常用的方式。

- `PAGE`：要求段的<span style="background:rgba(74, 82, 199, 0.2)">起始地址必须能被256整除</span>。例如，`12300H`。这通常用于大型数据段或代码段，以便于操作系统进行页面级别的内存管理和保护。

**“类名”和“使用类型”**：

1. **类名 (Class Name)**
    
    - **定义**：用单引号括起来的字符串，如`'CODE'`、`'DATA'`。
		
    - **作用**：在连接定位阶段，<span style="background:rgba(74, 82, 199, 0.2)">具有相同类名的逻辑段会被组合在一起，形成一个完整的物理段</span>。这是实现程序“模块化”设计的关键。
		
    - **模块化程序设计**：程序员可以将一个大型程序分解为多个源文件，每个源文件中定义自己的数据段或代码段，并赋予它们相同的类名。在链接时，<span style="background:rgba(74, 82, 199, 0.2)">链接器会自动将这些分散的逻辑段合并成一个大的物理段</span>，从而实现了代码的模块化管理和重用。

2. **使用类型 (Use Type)**（仅在386及以上处理器架构中有效）
    
    - `USE16`（缺省类型）：表示该段采用16位寻址方式。这意味着段基址和段内偏移量都是16位。这是传统的实模式或保护模式下的16位代码段的标准配置。
	
    - `USE32`：表示该段采用32位寻址方式。这意味着段选择子是16位，但段内偏移量是32位。这允许程序访问更大的内存空间（4GB），是保护模式下32位应用程序的标准配置。
	
    - `USE16`和`USE32`决定了段内指令和数据的寻址能力，而不是段本身的大小。一个`USE32`段可以比一个`USE16`段大得多，因为它支持32位的偏移量。

**组合方式**：

用于指定同类名段的组合方法。这是链接器在生成最终可执行文件时必须遵循的规则。

- `PRIVATE`（缺省方式）：不组合。每个具有相同类名的段都保持独立，不会与其他同名段合并。这是最简单的方式，适用于不需要跨模块共享数据或代码的场景。

- `PUBLIC`：同类名段按照前后次序连接在一起。这是最常用的组合方式，用于构建连续的代码段或数据段。例如，一个主程序模块和一个库模块都定义了名为`'CODE'`的段，链接器会将它们首尾相连，形成一个大的代码段。

- `COMMON`：与其他模块中的同类名段有相同的起始物理地址。这是一种“覆盖”机制，常用于定义全局变量。多个模块中定义的同名`COMMON`段，实际上指向同一块内存区域，从而实现了数据的共享。

- `STACK`：与其他模块中的同类名段用覆盖的方式连接，但从高地址开始覆盖。这是专门为堆栈段设计的组合方式。因为堆栈是从高地址向低地址增长的，所以这种组合方式可以确保所有模块的堆栈空间在内存中是连续且正确的。

- `MEMORY`：该段必须放在同类名的各个段中的最后。这通常用于定义程序的结束标志或保留区，确保其位于内存的最高端。

- `AT 表达式`：直接指定该段的段地址。这是一种非常规的、强制性的定位方式，通常用于编写操作系统内核或引导程序，需要将代码或数据精确地加载到某个特定的物理地址。

###### 假定伪指令

`ASSUME`是汇编语言中用于建立程序段与段寄存器之间逻辑关联的关键工具。

- **指令格式**：`ASSUME 段寄存器: 段名 [, 段寄存器: 段名, ...]`。

- **指令功能**：**告诉汇编程序段与段寄存器的对应关系**。在x86架构中，内存访问需要通过段寄存器（如CS, DS, SS, ES）来提供段基址，再结合偏移量来计算物理地址。`ASSUME`伪指令的作用是在汇编阶段告知汇编器，在后续的代码中，当遇到某个段寄存器时，它应该默认指向哪个逻辑段。

这是一个“假定”或“声明”，并非实际的硬件操作。它影响的是汇编器如何生成机器码中的地址信息。<span style="background:rgba(74, 82, 199, 0.2)">可以一次为多个段寄存器指定对应的逻辑段</span>。
...
> ```
>
> - 此例中，`ASSUME CS: CODE` 告诉汇编器，代码段寄存器`CS`将指向名为`CODE`的逻辑段。因此，当汇编器看到一条`JMP BEGIN`指令时，它知道`BEGIN`这个标号位于`CODE`段内，从而生成正确的相对跳转地址。
> - `ASSUME DS: DATA` 告诉汇编器，数据段寄存器`DS`将指向名为`DATA`的逻辑段。因此，当汇编器看到一条`MOV AL, VAR1`指令时，它知道`VAR1`位于`DATA`段内，从而在生成机器码时，会隐含地使用`DS`寄存器作为段基址。
> - `ASSUME`只是在汇编阶段建立一种逻辑上的关联。在程序运行时，CPU并不会自动加载这些段寄存器。程序员必须在程序开始执行时，手动使用`MOV AX, DATA`和`MOV DS, AX`等指令，将`DATA`段的段地址实际加载到`DS`寄存器中，才能使`ASSUME`的声明生效。

###### 定义符号名伪指令

`LABEL`伪指令：为当前的存储单元定义一个具有特定类型属性的符号名。

1. **核心功能**
    
    - 功能是**为当前的存储单元定义一个符号名，并指定其类型属性**。这里的“当前存储单元”指的是在汇编器处理到`LABEL`语句时，地址计数器所指向的那个位置。
    - 它允许程序员为同一块内存区域赋予不同的类型解释，从而实现灵活的数据访问。
2. **基本语法**
    
    - 格式为：`变量名或标号 LABEL 类型属性`。
    - 类型属性可以是`BYTE`、`WORD`、`DWORD`等，指定了该符号名所代表的数据宽度。
3. **等价性与应用示例**
    
    - PPT指出：“LABEL” = “EQU THIS”。这意味着`LABEL`伪指令的功能等同于先用`EQU`为当前地址定义一个别名，然后再指定其类型。
    - 示例：
        
        
    - 在这个例子中，`DATA_W`和`DATA_B`指向的是同一个内存起始地址。
        - `DATA_B`被定义为字节（Byte）类型，表示它是一个包含100个字节的数组。
        - `DATA_W`被定义为字（Word）类型，表示从同一个地址开始，它被视为一个由50个字组成的数组（因为1个字=2个字节）。
    - 因此，以下两个语句是等效的：
...
- **关键点**：`LABEL`本身不分配新的存储空间，它只是给已存在的存储单元（由其后的`DB`、`DW`等指令分配）赋予了一个新的、带有类型信息的符号名。

#### 编写完整的汇编语言程序

8086汇编程序设计的两个基本维度：

1. **静态程序框架**: 汇编语言源程序在文本上被<span style="background:rgba(74, 82, 199, 0.2)">组织（Organization）</span>，即`SEGMENT`的划分、`ASSUME`的约定以及`CODE`的初始化。
    
2. **动态构建过程**: 文本源程序（`.asm`）通过一系列工具（Toolchain）被转换为可执行文件（`.exe`）的。

#### 汇编语言程序设计

##### 程序设计基本方法

<span style="background:rgba(74, 82, 199, 0.2)">自顶向下的（Top-Down）</span>设计过程，包含四个关键阶段：

1. **分析实际问题，确定基本思路及程序算法**:
    
    - 此阶段是**需求分析**与**算法设计**。它要求开发者精确定义问题：需要什么输入（Inputs）、期望什么输出（Outputs），以及实现从输入到输出的转换所需的确定性步骤（即**算法 Algorithm**）。
        
2. **绘制程序流程图**:
    
    - 此阶段是**可视化设计**。将上一步中抽象的算法，使用标准化的图形符号（流程图）进行具象化、无歧义的描述。这是一种独立于任何特定编程语言的“通用设计蓝图”。
        
3. **根据流程图编写程序**:
    
    - 此阶段是**编码实现 (Implementation)**。开发者将流程图中的每一个符号，“翻译”成特定编程语言（如本课程中的汇编语言）的对应语句。
        
4. **调试程序**:
    
    - 此阶段是**验证与确认 (Verification & Validation)**。通过测试来确保实现的代码逻辑与流程图的设计一致，并且流程图的设计（即算法）能够正确解决最初分析的实际问题。

###### 流程图 (Flowchart)

流程图是算法的可视化表示法，它由以下几种基本符号构成：

- **起止框 (Terminal)**:（圆角矩形）表示一个程序、模块或子程序的**入口点**（开始）或**出口点**（结束）。
    
- **工作框 (Process)**:（矩形）表示一个或一系列确定的操作，如数据传送 (`MOV`)、算术运算 (`ADD`) 或任何非判断性的数据处理。这对应于**顺序结构**。
    
- **判断框 (Decision)**:（菱形）代表一个**条件判断**。它必须有一个入口连线，但有_两个或多个_出口连线（例如：是/否，真/假）。这是**分支结构**和**循环结构**的核心。
    
- **调用框 (Predefined Process)**:（带双竖边的矩形）代表对一个已定义的、独立的**子程序**或过程的调用。这是一个**抽象 (Abstraction)** 符号，它隐藏了子程序内部的复杂性。这直接对应于**子程序结构**。
    
- **连线 (Flowline)**:（箭头）表示程序的**控制流 (Control Flow)**，即指令的执行顺序。
...
- **定义**: 最基本的控制流。程序中的指令（程序段1, 程序段2, ...）<span style="background:rgba(74, 82, 199, 0.2)">按照它们在内存中存储的物理顺序依次执行</span>。
    
- **流程图对应**: 由一系列“工作框”通过“连线”串行连接而成。
    
- **硬件实现**: CPU中的**程序计数器 (Program Counter, PC)**（在x86中为 `IP` 指令指针）在每条指令执行完毕后，自动增加指向内存中的下一条指令，从而在硬件层面实现了顺序执行。
    

2. **分支结构 (Branch/Selection Structure)**

![alt text](image-47.png)

- **定义**: 允许程序的控制流根据一个**条件判断**的结果，从多个执行路径中选择一个。
    
- 核心是“判断框”。根据“条件?”的评估结果（Y或N），控制流被导向不同的“工作框”（程序段1或程序段2），并在最后**汇合**到一个共同的出口点。

> [!note] 案例分析
>
> ![alt text](image-48.png)
>
> 1. `CMP AL, BL`: (Compare) 这是实现判断的**前提工作**。它执行 `AL - BL` 操作，但_不_保存结果，而是根据结果（是否为0、是否为负等）来设置CPU的**标志寄存器 (Flags Register)**，特别是**零标志位 (Zero Flag, ZF)**。
>
> 2. `JZ L1`: (Jump if Zero) 这是真正的**条件判断**和**控制流转移**。它检查ZF：
>    - 如果 `ZF = 1`（Y, 真，表示 `AL == BL`），则`IP`寄存器被**强制修改**为 `L1` 标号的地址，控制流**跳转**到 `L1`。
>    - 如果 `ZF = 0`（N, 假，表示 `AL != BL`），则此指令不执行任何操作（NOP），控制流**顺序执行**到下一条指令。
>
> 3. `L2: ...... JMP NEXT`: 这是“N”（不等）的执行路径（“程序段2”）。`JMP NEXT` 是一个**无条件跳转**，它**必须**存在，以确保在执行完“不等”的逻辑后，能**跳过**“相等”的逻辑（`L1`），到达`NEXT`汇合点。
>
> 4. `L1: ......`: 这是“Y”（相等）的执行路径（“程序段1”）。
>
> 5. `NEXT: ......`: 这是两个分支的**汇合点**，程序从这里继续顺序执行。

> [!tip] 现代CPU的分支预测与性能优化
>
> 现代CPU采用**分支预测（Branch Prediction）** 技术来提升流水线执行效率。当遇到条件跳转（如 `JZ`）时，CPU会根据历史信息预测跳转结果，并**投机执行**预测路径上的指令。如果预测正确，性能最大化；若预测失败，则需清空流水线并重新取指，带来显著性能损失。因此，编写**易于预测的分支逻辑**是高性能软件开发的重要优化点。

3. **循环结构**

![alt text](image-49.png)

- **定义**: 允许一个程序段（循环体）在特定条件满足时被**重复执行**。
    
- **三个基本部分**:
    
    1. **初始化部分**: 在循环开始前_仅执行一次_，用于设置循环的初始状态（如计数器、指针）。
        
        - **汇编实例**: `MOV CX, 100`。在x86架构中，`CX` 寄存器被约定为<span style="background:rgba(74, 82, 199, 0.2)">循环计数器</span>。此处将其初始化为100。
            
    2. **循环体部分**: 每次循环时真正执行的工作。
        
        - **汇编实例**: `L: ......` (循环体内的具体工作) `DEC CX` (修改循环变量，`CX` 减 1)。
            
    3. **循环条件判断部分**: 每次循环体执行后（或执行前）进行判断，决定是继续循环还是退出循环。
        
        - **汇编实例**: `JNZ L` (Jump if Not Zero)。此指令检查由 `DEC CX` 操作设置的ZF。如果 `CX` 减 1 后_不为0_（Y, 真），则跳转回 `L:` 标号处，执行下一次循环。如果 `CX` 为 0（N, 假），则不跳转，控制流顺序向下执行，循环**终止**。
            

这是一种“**先执行、后判断**”的循环（至少执行一次），这在高级语言中等价于 `do-while` 循环。而x86的 `LOOP` 指令（`LOOP L`）则是一个 `DEC CX` 和 `JNZ L` 的**复合指令**。

> [!tip] 循环结构的工业优化与前沿技术
>
> - **编译器循环优化**：循环是性能优化的重点。常见技术包括：
>   - **循环展开（Loop Unrolling）**：将循环体复制多份，减少分支判断和跳转指令的开销，提升执行效率。
>   - **循环不变代码外提（Loop-invariant Code Motion）**：将循环中不会变化的计算提前到循环外，避免重复执行。
> - **SIMD并行加速**：现代CPU支持SIMD指令集（如SSE、AVX、NEON），可在一次指令中并行处理多个数据元素，大幅提升如图像处理、AI等场景下的循环性能。

4. **子程序结构 (Subroutine Structure)**

![alt text](image-50.png)

- **定义**: 子程序（也称过程 `Procedure` 或函数 `Function`）是一个**可复用**的、封装了特定功能的独立代码块。主程序可以在任何需要的时候**调用 (Call)** 它，执行完毕后，它会**返回 (Return)** 到调用处的下一条指令。
    
- **优势**:
    
    - **模块化 (Modularity)**: 将大型程序分解为小的、高内聚、低耦合的模块。
        
    - **复用性 (Reusability)**: "Don't Repeat Yourself" (DRY)。一段逻辑只需编写一次，可以被任意次调用。
        
    - **可维护性 (Maintainability)**: 修改一个子程序，所有调用它的地方都会自动获得更新。
    
子程序的调用和返回并不是一个简单的 `JMP`，而是需要被精密管理，涉及以下点：

1. **寄存器的保护与恢复**:
    
    ![alt text](image-51.png)

    - **问题**: 主程序和子程序都可能使用同一组寄存器（如 `AX`, `BX`）。<span style="background:rgba(74, 82, 199, 0.2)">子程序不能破坏主程序正在使用的寄存器值</span>。
    
    - **机制**: **调用者保存 (Caller-Saved)** 或 **被调用者保存 (Callee-Saved)** 规约。一种常见的做法是，子程序在开始时使用 `PUSH` 指令将其需要使用的寄存器值**压入堆栈 (Stack)**，在返回（`RET`）前，再使用 `POP` 指令按相反顺序**弹出堆栈**，恢复寄存器的原始值。
        
2. **主程序与子程序之间的参数传递**:
    
    ![alt text](image-52.png)

    - **问题**: 主程序如何将数据（如 `AL` 和 `BL`）传递给子程序进行处理？子程序如何将结果（如“和”或“差”）返回给主程序？
        
    - **机制**: 这是由**应用程序二进制接口 (Application Binary Interface, ABI)** 定义的。常见方式包括：
        
        - **寄存器传递**: 约定使用特定寄存器（如 `AX`, `BX`）传递参数和返回值。速度快，但数量有限。
            
        - **堆栈传递**: 主程序将参数 `PUSH` 到堆栈上，子程序通过访问堆栈（如 `[BP+4]`）来获取参数。
            
3. **子程序可嵌套调用**:
    
    ![alt text](image-53.png)

    - **机制**: 子程序A可以调用子程序B。这一机制完全依赖于**堆栈**。
        
    - **硬件支持**:
        
        - `CALL L1` 指令: CPU自动将下一条指令的地址（**返回地址**）`PUSH` 到堆栈，然后 `JMP` 到 `L1`。
            
        - `RET` 指令: CPU自动从堆栈 `POP` 出顶层的**返回地址**，然后 `JMP` 回这个地址。
            
    - 正是由于堆栈“后进先出”(LIFO)的特性，使得`A`调用`B`，`B`调用`C`后，`C`可以`RET`回`B`，`B`再`RET`回`A`，保证了<span style="background:rgba(74, 82, 199, 0.2)">调用链</span>的正确返回。
        

> [!tip] 子程序的工业与前沿技术应用
>
> - **堆栈帧 (Stack Frame)**：现代高级语言每次函数调用都会在堆栈上分配“堆栈帧”，由 `BP`（基址指针）和 `SP`（栈顶指针）管理。堆栈帧用于存储返回地址、保存的寄存器、参数和局部变量，是实现递归、嵌套调用和多线程安全的基础。
> 
> - **API（应用程序编程接口）**：子程序是API的基本单元。操作系统和各类库（如Windows API、OpenGL）通过一系列函数（子程序）向开发者提供服务，促进了软件的模块化和复用。
> 
> - **微服务（Microservices）**：在云原生和分布式架构中，微服务可视为跨主机的“子程序”。通过网络协议（如HTTP/gRPC）进行调用，参数和结果通过JSON/Protobuf等格式传递。其本质仍是“抽象、封装、调用与返回”，与汇编的 `CALL`/`RET` 机制一脉相承。
> 
...
>[!note] `INT 21H` ：中断服务中用于显示单个字符的**原子I/O原语**。
>
> - **功能**: 将指定的字符送到显示器显示。这是一个底层的输出功能。
> - **调用规约 (Calling Convention)**: 这是一个**系统调用 (System Call)**，它要求在触发中断前，必须满足特定的寄存器状态：
>   - **`AH` 寄存器**: 必须设置为 `02H`。`AH` 在 `INT 21H` 中充当**功能号选择器**，`02H` 即代表“显示字符”服务。
>   - **`DL` 寄存器**: 必须设置为待显示字符的**ASCII码**。`DL` 在此作为**输入参数**。
...
>
> - `MOV DL, 'A'`:
>   - 此指令将字符 `'A'` 的ASCII码值（即 `41H`）加载到 `DL` 寄存器中。
>
> - `MOV AH, 02H`:
>   - 此指令设置功能号为 `02H`。
>
> - `INT 21H`:
>   - `INT` (Interrupt) 是一条**软件中断**指令。它将程序的控制权从当前的用户程序（User Mode）切换到操作系统内核（Kernel Mode）的**中断服务例程 (Interrupt Service Routine, ISR)**。
>   - DOS内核的 `21H` ISR 被激活后，它首先检查 `AH` 寄存器的值。
>   - 当它识别到 `AH = 02H` 时，它便执行对应的服务：从 `DL` 寄存器中读取 `41H`，并通过调用更底层的BIOS中断（如 `INT 10H`）或直接操作显存（Video RAM），将字符 'A' 显示在当前光标位置。
>   - 完成后，控制权通过 `IRET` (Interrupt Return) 指令返回到用户程序的下一条指令。

>[!note] `INT 21H` 字符串显示功能（功能号 09H）
>
> - **功能**: 用于将数据段中的字符串完整输出到屏幕。该功能将字符输出的迭代过程封装在操作系统内核中，实现更高级、更抽象的I/O操作。
> - **调用规约**:
>   - **`AH` 寄存器**: 必须设置为功能号 `09H`。
>   - **`DS:DX` 寄存器对**: 必须组成一个远指针（Far Pointer），指向要显示字符串的首地址。`DS`（Data Segment）存放段基址，`DX`（Data Index）存放偏移量。
>   - **数据结构要求**: 操作系统规定，所指向的字符串必须以 `'$'`（美元符号，ASCII `24H`）作为终止符，这是严格的接口契约。

>[!note] 案例分析
>
> ![alt text](image-55.png)
>
> - `DATA SEGMENT ... STRING DB 'HELLO', 0DH, 0AH, '$' ... DATA ENDS`:
>   - 在数据段中定义了符合规约的数据结构。`STRING` 变量包含了内容 ('HELLO')、换行符 (`0DH`=回车, `0AH`=换行) 以及 DOS 要求的 `'$'` 终止符。
>
> - `MOV AX, DATA` / `MOV DS, AX`:
>   - 这是初始化 `DS` 寄存器的标准两步操作。段寄存器不能被立即数（如 `DATA` 段的地址）直接赋值，必须通过一个通用寄存器（如 `AX`）中转。
>
> - `LEA DX, STRING`:
>   - `LEA` (Load Effective Address) 指令计算 `STRING` 标号的**偏移地址**（Offset），并将其加载到 `DX` 寄存器。
>
> - `MOV AH, 09H`:
>   - 设置功能号。
>
> - `INT 21H`:
>   - 触发中断。DOS内核的 `09H` 服务例程被激活。它从 `DS:DX` 所指向的地址开始，读取一个字节，调用原子I/O（类似 `02H` 的功能）显示它；然后将内部指针加1；接着读取下一个字节，检查它是否为 `'$'`。
>   - 这个“读-显示-递增-检查”的**内部循环**会一直持续，直到它读取到 `'$'` 字符，此时循环终止，服务例程返回。
...
在这种结构中，CPU通常作为**总线主控设备 (Bus Master)**，而内存和I/O接口作为**总线从属设备 (Bus Slaves)**。CPU通过这组总线来协调和控制所有的数据交换。
##### 总线的分类（按功能划分）

###### 地址总线 (Address Bus, AB)

地址总线（AB）的功能是**寻址 (Addressing)** 或 **选择 (Selection)**。当CPU需要从内存读取数据或向I/O端口写入数据时，首先需要指明它要操作的具体单元。CPU将该单元的唯一编号（即**地址**）放置到地址总线上。

**传输方向**: <span style="font-weight:bold; color:rgb(231, 98, 84)">地址总线是单向的</span>。在主-从架构中，<span style="background:#d2cbff">CPU（主设备）是地址的唯一发起者</span>，它始终是向外输出（Output）地址信号。存储器和I/O设备（从设备）始终是地址的接收者（Input），它们“侦听”地址总线，当发现总线上的地址与自身地址匹配时，才会响应后续的数据或控制信号。
    
- **寻址范围**: <span style="font-weight:bold; color:rgb(231, 98, 84)">地址总线的位数决定了寻址范围</span>。
    
    - **寻址能力**（N）与**地址总线宽度**（W）的关系是 $N = 2^W$。
        
    - **例**: Intel 8086 CPU有**20位**地址信号。因此，其物理寻址范围为 $2^{20}$ 个字节，即 1,048,576 字节，也就是 **1MB** 物理地址空间。

> [!tip] 拓展
> - 32位处理器（如80386, Pentium, ARMv7）具有32位地址总线，其寻址范围为 $2^{32}$ 字节，即 **4GB**。这是32位操作系统（如Windows XP, 32-bit Linux）所能管理内存的理论上限。
> - 现代64位处理器（x86-64, AArch64）的地址总线宽度在物理上通常并未实现完整的64位（$2^{64}$ 是一个天文数字），而是实现一个子集，例如48位或52位，这提供了高达256TB或4PB的物理寻址空间，足以满足当前和可预见的未来服务器市场的需求。

###### 数据总线 (Data Bus, DB) 

数据总线（DB）是用于在CPU、存储器和I/O设备之间**实际传送数据信息**的通道。

- **传输方向**: <span style="font-weight:bold; color:rgb(231, 98, 84)">双向传输 (Bidirectional)</span>。
    
    - **写操作 (CPU Write)**: CPU作为主设备，将数据（例如来自寄存器）输出到数据总线上，目标设备（如RAM）从总线读取该数据。
        
    - **读操作 (CPU Read)**: 目标设备（如ROM）作为响应，将数据输出到数据总线上，CPU从总线读取该数据。

**三态逻辑 (Tri-state Logic)**: <span style="font-weight:bold; color:rgb(231, 98, 84)">双向传输时通常采用双向三态形式</span>。由于多个设备（CPU, RAM, I/O）都可能需要向数据总线“输出”数据，为防止**总线冲突**（Bus Conflict，多个设备同时驱动总线导致信号短路），所有连接到数据总线的设备都必须具备 **三态（Tri-state）** 输出能力（即高电平、低电平、高阻态 HZ）。当设备未被选中时，其数据总线接口处于**高阻态**，相当于在电气上与总线断开。

![alt text](image-59.png)

**例**：此电路的目的是从四个输入源（`A0`, `B0`, `C0`, `D0`）中**选择一个**，将其数据发送到共享总线上面。

控制逻辑（例如CPU）设置 `Select` 线（`S1`, `S0`）的值。假设设置为 `S1=1`, `S0=0`（二进制的 `10`，即十进制的 `2`），并设置 `E` 为使能。

译码器（黑盒，也就是 2-to-4 译码器）接收到 `10`，于是它**激活**其 `2` 号输出线（使其为高电平），同时保持 `0`, `1`, `3` 号输出线为低电平。而译码器的 `0`, `1`, `3` 号输出线（均为低电平）分别连接到 `A0`, `B0`, `D0` 所属的缓冲器（三角符号）。这三个缓冲器接收到非激活的控制信号，它们**全部进入高阻态 (Hi-Z)**。

- **数据总线宽度**: <span style="font-weight:bold; color:rgb(231, 98, 84)">位数通常与微处理器的字长一致</span>（16位处理器、32位处理器）。

- **例**: 8086微处理器字长为16位，其数据总线宽度也是16位。这意味着8086在**一个总线周期 (Bus Cycle)** 内可以传送16 bits（2字节，一个字）的数据。

> [!tip] 拓展
> 
> 现代64位CPU（如Core i9, AMD Ryzen）的数据总线宽度为64位（8字节）。为了进一步提高**内存带宽 (Memory Bandwidth)**，它们还采用多通道内存技术（如DDR5双通道），这在CPU的集成内存控制器（IMC）和DRAM模组之间创建了一个等效的128位宽的数据通路。
>    
> **总线复用**: 为了节省CPU的物理引脚数量，8086/8088采用了**地址/数据分时复用 (Address/Data Multiplexing)** 技术。
>    
> 这需要**控制总线**上的`ALE` (Address Latch Enable) 信号配合**外部地址锁存器**（如74LS373）才能将复用的信号（`AD0-AD15`）“解复用”为独立的地址总线（来自锁存器输出）和数据总线。

###### 控制总线 (Control Bus, CB)

决定“**做什么**”和“**何时做**”。它是CPU用于**协调和同步**所有总线操作的信号集合。

- **信号组成**: 控制总线并非一个同质化的总线，而是**一组异质化（heterogeneous）的控制信号线**的统称。
    
    - **PPT实例**: `读/写命令`：例如 `RD#` (读选通) 和 `WR#` (写选通) 信号。CPU通过拉低这些信号线来通知内存或I/O设备执行读或写操作。
        
    - **PPT实例**: `中断请求`：例如 `INTR` (Interrupt Request)。这是I/O设备向CPU发起，用于“打断”CPU当前工作以请求服务的信号。
        
    - **其他关键信号**: 还包括 `M/IO#`（用于区分是访问内存还是I/O）、`ALE`（如上所述，用于地址锁存）、`READY`（用于慢速设备通知CPU插入**等待状态 (Wait States)**）以及用于**总线仲裁 (Bus Arbitration)** 的信号（如 `HOLD`/`HLDA`，用于DMA）。
        
- **传输方向与位数**: `传送方向由具体控制信号来定，其位数也由系统的实际控制需要来定`。
    
    - `RD#`/`WR#` 是**单向**的（CPU -> 设备）。
        
    - `INTR`/`READY` 是**单向**的（设备 -> CPU）。
        
    - `HOLD`/`HLDA` 则是**双向交互**的。
        
    - 因此，控制总线整体既非单向也非双向，其总线宽度（位数）是所有必需控制线的简单加和。
#### 几种常用的芯片
